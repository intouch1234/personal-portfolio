  ...
  return (
    ...
    className='my-20'>
      ...
      <div className='flex flex-col items-center justify-center'>
        ...
        className='relative w-[800px] lg:w-[600px] md:w-[95%] sm:w-[280px] h-[500px] lg:h-[450px]
        md:h-[400px] sm:h-[600px] flex items-center justify-center overflox-hidden'>
          {reviewsData.map((review, i)=>(
                      <motion.div 
                      initial={{x:'100%'}}
                      key={i}
                      className='absolute inset-0 flex flex-col items-center justify-center gap-y-7 lg:gap-y-4
                      border border-yellow-500 bg-zinc-50 p-14 lg:p-5 rounded-xl
                      dark:bg-zinc-700 transition-color'
                      ref={(el)=> slides.current.push(el)}>
                        <Image src={review.image} alt='Reviews Image' width={130} height={130}
                        ...
                        <p className='text-lg md:text-sm text-justify font-extralight tracking-wide text-gray-600
                        first-leter:pl-2
        ...
        <div className='flex gap-x-4 text-4xl text-yellow-500 mt-5'>
          ...
          className={`${index===0 ? 'opacity-30 pointer-events-none': 
            'opacity-100 pointer-events-auto'} hover:scale-150 transition-all`}
          onClick={()=>{
            setDirection(true)
            setIndex(index-1)
          }}>{arrowIcons[0]}</button>
          <button 
          className={`${index===reviewsData.length-1 ? 'opacity-30 pointer-events-none': 
            'opacity-100 pointer-events-auto'} hover:scale-150 transition-all`}
          onClick={()=>{
            setDirection(false)
            setIndex(index+1)
