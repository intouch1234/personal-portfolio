...
const Toggle = ({children}) => {
    ...
    useEffect(()=>{
        ...
        const systemTheme = 
        typeof window !== undefined && window.matchMedia('(prefers-color-scheme:dark').addEventListener.matches

        if(darkThemeParsed === undefined) {
  ...
  return (
    ...
    >
      <div
      className='bg-zinc-50 dark:bg-zinc-800 transition-color'>
        <div
        className='max-w-[1200px] xl:w-full mx-auto flex justify-center xl:px-[90px] sm:pl-[80px] sm:pr-5
        overflow:hidden'>
        <button 
        onClick={()=>{
            if(!darkTheme) {
        ...
        className='fixed right-14 sm:right-10 top-10 text-yellow-600 hover:text-yellow-500 z-40'>
            <motion.span animate={{scale:darkTheme ? 0 : 1}} 
            className='absolute block rounded-full bg-zinc-50 p-1 text-4xl
            dark:bg-zinc-800'>
                {moonIcon}
            </motion.span>
            <motion.span animate={{scale:darkTheme ? 1 : 0}} 
