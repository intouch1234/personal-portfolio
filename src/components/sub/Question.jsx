...
const Question = ({data,index}) => {
    ...
    const variants = {
        visible: (i) => ({opacity:1,x:0,transition:{
            delay: i*0.07
        },}),
  ...
  return (
<motion.li 
custom={index} initial='hidden' whileInView='visible' viewport={{margin:'50px', once:true}}
variants={variants}
className="border border-yellow-500 p-1 rounded-lg">
    ...
    animate={{scaleY: show? 1:0, height: show? 'auto' : 0,
        opacity: show? 1:0
    }}
    transition={{duration:0.1,type:'spring', stiffness: show ? 250 : 50, opacity:{delay:show?0.2:0}}}
    className="box=border origin-top pl-8 text-lg font-extralight 
    tracking-wide text-gray-900 first-letter:pl-3
    dark:text-gray-200">
      {data.answer}
    </motion.p>
</motion.li>
